...
class JsonSchemaTestSuiteTest extends TestCase
{
            ...
            foreach ($files as $file) {
                ...
                foreach ($contents as $testCase) {
                    foreach ($testCase->tests as $test) {
                        $filename = str_replace('\\', '/', preg_replace('#^.*[/\\\\]tests[/\\\\]#', '', $file->getRealPath()));
                        $name = sprintf(
    ...
    private function loadRemotesIntoStorage(SchemaStorageInterface $storage): void
    {
        ...
        foreach ($iterator as $info) {
            ...
            $relativePath = str_replace('\\', '/', substr($info->getPathname(), strlen($remotesDir)));
            $id = 'http://localhost:1234' . $relativePath;
    ...
    private function shouldNotYieldTest(string $name): bool
    {
        $skip = [
            ...
            // Optional: bignum â PHP does not natively support arbitrary-precision integers/floats
            ...
            // Optional: float-overflow â PHP float precision differs from the ECMAScript model
            ...
            // Optional: ecmascript-regex â PHP uses PCRE which does not implement ECMAScript regex semantics
            ...
            // Optional: cross-draft â cross-draft schema resolution ($ref across drafts) is not implemented
            ...
            // Optional: idn-email â IDN e-mail format validation is not implemented
            ...
            // Optional: idn-hostname â IDN hostname format validation is not implemented
            ...
            // Optional: iri / iri-reference â IRI format validation is not implemented
            ...
            // Optional: regex format â regex format validation does not check for valid ECMA-262 regex syntax
            ...
            // Optional: relative-json-pointer â relative JSON pointer format validation is not implemented
