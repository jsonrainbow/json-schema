...
class CollectionConstraint extends Constraint
{
    ...
    public function check($value, $schema = null, $path = null, $i = null)
    {
        ...
        if (isset($schema->minItems) && count($value) < $schema->minItems) {
            $this->addError($path, "There must be a minimum of " . $schema->minItems . " in the array", 'minItems', array('minItems' => $schema->minItems,));
        }

        // Verify maxItems
        if (isset($schema->maxItems) && count($value) > $schema->maxItems) {
            $this->addError($path, "There must be a maximum of " . $schema->maxItems . " in the array", 'maxItems', array('maxItems' => $schema->maxItems,));
        }
        ...
        if (isset($schema->uniqueItems)) {
            ...
            if (count(array_unique($unique)) != count($value)) {
                $this->addError($path, "There are no duplicates allowed in the array", 'uniqueItems', array('uniqueItems' => $schema->uniqueItems,));
            }
        }
